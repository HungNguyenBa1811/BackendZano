...
const { StudentExamsRepository } = require('./student-exams.repository');
...
class StudentExamsService {
	async listExams(query) {
		...
		if (type && !['midterm', 'final', 'practice'].includes(type)) {
			throw new AppError('Du lieu khong hop le', 400, 'VALIDATION_ERROR', [{ field: 'type', issue: 'invalid_enum' }]);
		}

		if (status && !['open', 'closed'].includes(status)) {
			throw new AppError('Du lieu khong hop le', 400, 'VALIDATION_ERROR', [{ field: 'status', issue: 'invalid_enum' }]);
		}
...
module.exports = { StudentExamsService };
